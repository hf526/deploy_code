...
pub fn run() {
    ...
    app.run(|app_handle, event| {
        if let RunEvent::ExitRequested { api, code, .. } = event {
            if code.is_none() && !EXIT_CLEANUP_STARTED.swap(true, Ordering::SeqCst) {
                let pending = app_handle.state::<AppState>().take_deploys();
                if !pending.is_empty() {
                    // 有部署仍在进行：推迟退出，先终止远端脚本（有超时兜底）。
                    api.prevent_exit();
                    let handle = app_handle.clone();
                    tauri::async_runtime::spawn(async move {
                        // 先中止本地部署任务，防止清理期间它再启动新脚本。
                        for (_, deploy) in &pending {
                            deploy.abort.abort();
                        }
                        // 留一点时间给已在途的远端脚本写完 pidfile。
                        tokio::time::sleep(Duration::from_millis(400)).await;
                        cleanup_remote_deploys(&handle, pending).await;
                        handle.exit(0);
                    });
                }
            }
...
/// 逐个重连服务器，终止仍在运行的部署脚本进程组；总时长有上限，避免退出卡死。
async fn cleanup_remote_deploys(
    app: &tauri::AppHandle,
    pending: Vec<(String, state::ActiveDeploy)>,
) {
    ...
    let cleanup = async {
        for (record_id, active) in &pending {
