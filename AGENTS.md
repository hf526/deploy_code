...
- 所有用户可见文案必须走 `t("...")`，并**同时**在 `src/locales/zh-CN.json` 与 `en-US.json` 增加同名 key（两边必须 1:1，当前 893 对全对齐）。两份文件是 2 空格缩进 + CRLF，用脚本改时先确认 `json.dumps(..., indent=2, ensure_ascii=False)` 能原样往返再写回（读文件要用二进制或 `newline=""`：文本模式会把 CRLF 悄悄折成 LF，往返检查看着通过、写回却改掉整份文件的行尾）。状态文案/尺寸/耗时格式化用 `src/lib/utils.ts`。
